...
using WebApi.Seeders.PostOfficeHelpers.Request;
...
public static class DataSeeder {
	public static async Task SeedNecessaryDataAsync(this IApplicationBuilder builder) {
		...
		await SeedIdentity(service, context);
	}

	public static async Task SeedPostOfficesAsync(this IApplicationBuilder builder) {
		using var scope = builder.ApplicationServices.GetRequiredService<IServiceScopeFactory>().CreateScope();
		var service = scope.ServiceProvider;
		var context = service.GetRequiredService<DataContext>();
		...
		await Console.Out.WriteLineAsync("Seeding PostOffices started...");

		IConfiguration configuration = service.GetRequiredService<IConfiguration>();
		IMapper mapper = service.GetRequiredService<IMapper>();
		string novaPoshtaKey = configuration["NovaPoshtaKey"]
			?? throw new Exception("NovaPoshtaKey not found in appsettings.json");

		AggregateResponse aggregateData = await LoadDataFromApiAsync(novaPoshtaKey);
		List<AreaVm> areas = CreateRelationshipBetweenData(aggregateData, mapper);
		List<Area> areaEntities = MapToDbEntities(areas);

		await context.Areas.AddRangeAsync(areaEntities);
		await context.SaveChangesAsync();

		await Console.Out.WriteLineAsync("Seeding PostOffices completed");
	}

	public static async Task SeedTestCategoriesAndProducts(this IApplicationBuilder builder) {
		using var scope = builder.ApplicationServices.GetRequiredService<IServiceScopeFactory>().CreateScope();
		var service = scope.ServiceProvider;
		var context = service.GetRequiredService<DataContext>();
		var seeder = service.GetRequiredService<IContentSeeder>();
		...
		if (!await context.Categories.AnyAsync()) {
			...
			foreach (var category in createdCategories) {
				...
				try {
					await seeder.SeedProductsAsync(id, 100);
				}
				catch {
					await Console.Out.WriteLineAsync($"Error creating products for category with id {id}");
				}
			}
		}
	}
	...
	private static async Task SeedIdentity(IServiceProvider service, DataContext context) {
	...
	private static async Task CreateAdminAsync(UserManager<User> userManager) {
		...
		var user = new User {
			...
			UserName = "Lite",
			Photo = await ImageWorker.SaveImageAsync(base64Image)
		};

		IdentityResult result = await userManager.CreateAsync(user, "admin");

		if (!result.Succeeded)
			throw new Exception("Error creating administrator account");

		await userManager.AddToRoleAsync(user, Roles.Admin);
	}


	private static async Task<T> GetResponseAsync<T>(string url, RequestData data) {
		...
		if (!response.IsSuccessStatusCode)
			throw new Exception("Not success status code");

		var responseData = await response.Content.ReadAsStringAsync();
		return JsonConvert.DeserializeObject<T>(responseData)
			?? throw new Exception("Returned null after deserialization of the response");
	}

	private static async Task<AggregateResponse> LoadDataFromApiAsync(string novaPoshtaKey) {
		RequestData requestAreas = new() {
		...
		RequestData requestSettlements = new() {
			ApiKey = novaPoshtaKey,
			ModelName = "Address",
		...
		RequestData requestPostOffices = new() {
			ApiKey = novaPoshtaKey,
			ModelName = "Address",
		...
		var areasTask = GetResponseAsync<ResponseData<AreaData>>("https://api.novaposhta.ua/v2.0/json/", requestAreas);
		var settlementsTask = GetResponseAsync<ResponseData<SettlementData>>("https://api.novaposhta.ua/v2.0/json/", requestSettlements);
		var postOfficesTask = GetResponseAsync<ResponseData<PostOfficeData>>("https://api.novaposhta.ua/v2.0/json/", requestPostOffices);

		await Task.WhenAll(areasTask, settlementsTask, postOfficesTask);

		var areasResponse = areasTask.Result;
		var settlementResponse = settlementsTask.Result;
		var postOfficesResponse = postOfficesTask.Result;

		if (areasResponse.Success != "true" || settlementResponse.Success != "true" || postOfficesResponse.Success != "true")
			throw new Exception("One or more responses are not successfully");

		return new AggregateResponse {
			Areas = [.. areasResponse.Data],
			Settlements = [.. settlementResponse.Data],
			PostOffices = [.. postOfficesResponse.Data]
		};
	}

	private static List<AreaVm> CreateRelationshipBetweenData(AggregateResponse aggregateData, IMapper mapper) {
		List<AreaVm> areas = aggregateData.Areas
			.Select(mapper.Map<AreaVm>)
			.ToList();

		Dictionary<string, List<SettlementVm>> areaSettlements = [];
		foreach (SettlementData s in aggregateData.Settlements) {
			string area = s.Area;

			var vm = mapper.Map<SettlementVm>(s);

			if (!areaSettlements.TryGetValue(area, out List<SettlementVm>? value))
				areaSettlements.Add(area, [vm]);
			else
				value.Add(vm);
		}

		Dictionary<string, List<PostOfficeVm>> cityOffices = [];
		foreach (PostOfficeData po in aggregateData.PostOffices) {
			string cRef = po.CityRef;

			var vm = mapper.Map<PostOfficeVm>(po);

			if (!cityOffices.TryGetValue(cRef, out List<PostOfficeVm>? value))
				cityOffices.Add(cRef, [vm]);
			else
				value.Add(vm);
		}

		foreach (var keyValuePair in areaSettlements) {
			var value = keyValuePair.Value;

			value.ForEach(v => {
				v.PostOffices = cityOffices.GetValueOrDefault(v.Ref) ?? [];
			});
		}

		areas.ForEach(v => {
			v.Settlements = areaSettlements.GetValueOrDefault(v.Ref) ?? [];
		});

		return areas;
	}

	private static List<Area> MapToDbEntities(List<AreaVm> areas) {
		return areas.Select(a => new Area {
			Name = a.Description,
			Settlements = a.Settlements.Select(s => new Settlement {
				Name = s.Description,
				PostOffices = s.PostOffices.Select(p => new PostOffice {
					Name = p.Description,
